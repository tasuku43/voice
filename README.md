...
The current core implements dictionary models, normalization, local learning sources, generated learning entries for the local context model, JSON persistence, fixtures, and evals. A first Apple Speech adapter exists behind the replaceable STT protocol.

The current app shell includes a minimal macOS menu bar executable with a configurable global voice-input hotkey (default Control-Option-Space), press-and-hold recording, AVFoundation microphone recording, permission status display, voice input permission shortcut, on-device Apple Speech transcription, Accessibility-based paste, pasteboard fallback, and a `Model Education` submenu for local context model rebuild/export/import/delete actions.
...
- macOS app shell: AppKit menu bar utility
...
Use the menu bar `Hotkey Settings...` item to change the voice-input key and modifiers locally. Use `Permission Status...` to inspect microphone, speech recognition, Accessibility paste, and Input Monitoring states before a real recording run, and `Open Voice Input Permissions...` to open the missing macOS privacy controls for hotkeys and automatic paste. Hotkey settings, local context model data, and repository path are stored under Application Support; `Model Education` -> `Open Local Data Folder...` reveals that location for manual privacy checks. Raw audio is temporary and raw transcripts are not persisted by default.
...
Component contracts live under `docs/contracts/`, and short next-session prompts live under `docs/codex-sessions/`. Use those when continuing focused work on speech, normalization, repository vocabulary, learning, or output.
