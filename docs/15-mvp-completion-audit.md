...
| Local context model boundary | `LocalContextModel`, `LocalContextModelBuildUseCase`, `LocalContextModelDataUseCase.rebuildModel`, `LocalContextModelDocumentCodec`, `JSONLocalContextModelRepository`, `DictionaryEntryLoadingUseCase`, app model export/import/delete menu actions, `docs/contracts/local-context-model.md`, `docs/codex-sessions/local-context-model-session.md`, `SpeechRecognitionHintsUseCase`, learning source tests | Implemented with a versioned local JSON document; training rebuilds the saved model, hotkey runtime loads saved post-STT entries, and the app can export/import/delete the model |
