...
| Keep candidate generation explainable while separating it from the hotkey app layer. | `CorrectionCandidate`, `CandidateExtractor`, `LocalContextCandidateGenerationUseCase`, and repository vocabulary learning generate reasons and confidence metadata for model education. Prompt insertion carries only text; it does not return learning candidates. | Implemented |
