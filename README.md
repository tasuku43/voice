...
The current app shell includes a minimal macOS menu bar executable with a Command-Shift-Space hotkey trigger, AVFoundation microphone recording, on-device Apple Speech transcription, preview window, pasteboard insertion, local approval of dictionary candidates, and simple in-progress state for the recording flow.
