...
| Keep candidate generation explainable while separating it from the hotkey app layer. | `CorrectionCandidate`, `CandidateExtractor`, `AgentHistoryDictionaryLearningUseCase`, and repository vocabulary learning generate reasons and confidence metadata for model education. Prompt confirmation returns only the prompt to insert; it does not return learning candidates. | Implemented |
| Detect likely voice misrecognitions behind a replaceable detector. | `VoiceMisrecognitionDetector`, `RuleBasedVoiceMisrecognitionDetector`, and candidate extractor tests cover replaceable detector behavior for model education. | Implemented |
...
| Keep dangerous substitutions from auto-applying. | `DangerousCommandPolicy`, `CandidateExtractor`, and rule-based detector tests keep dangerous command candidates explainable and non-auto-applied. | Implemented |
