...
| Learn environment-specific vocabulary from local Codex and Claude history plus configured repository vocabulary. | `LearningSource`, `LearningSourceSelection`, `LocalAgentHistoryTextProvider`, `RepositoryVocabularyLearningSource`, `AgentHistoryLearningModeUseCase`, `AgentHistoryDictionaryLearningUseCase`, menu items `Train Dictionary From Sources...` and `Learn From Agent History...`, and tests for bounded local history loading, structured user text extraction, source text counts, candidate generation, duplicate skipping, repository vocabulary as a learning source, and project identifier candidates. | Implemented |
