...
The current implementation starts with `LocalContextModel` and `LocalContextModelBuildUseCase`, which combine seed entries and generated learning entries into one model. `LocalContextModelRebuildUseCase.rebuild(...)` runs selected learning sources, generates entries, and saves the rebuilt model through `LocalContextModelDataUseCase.rebuildModel(...)`. `LocalContextModelDocumentCodec` stores the model in a versioned JSON document, `JSONLocalContextModelRepository` is the local filesystem adapter, and `DictionaryEntryLoadingUseCase` loads saved `postSTTEntries` into the hotkey runtime.
