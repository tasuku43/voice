...
| Local context model boundary | `LocalContextModel`, `LocalContextModelBuildUseCase`, `docs/contracts/local-context-model.md`, `docs/codex-sessions/local-context-model-session.md`, `SpeechRecognitionHintsUseCase`, learning source tests | Implemented in memory; persistent model document remains future work |
